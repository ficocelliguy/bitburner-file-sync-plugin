...
import { BASE_COST_LABEL, BASE_RAM_COST, computeScriptRamCost, formatRam, parseRamCosts } from '../../ram/RamCost';
...
export async function main(ns) {
    ...
    await ns.grow("n00dles");
        ...
        assert.equal(result.total, 0.1 + 0.15 + BASE_RAM_COST);
        assert.deepEqual(
            result.entries.map(e => e.name),
            ['grow', 'hack', BASE_COST_LABEL],
        );
    });
    ...
    test('sorts ns entries highest cost first, then appends Base cost last', () => {
        ...
        assert.deepEqual(result.entries, [
            ...
            { name: 'hack', cost: 0.1 },
            { name: BASE_COST_LABEL, cost: BASE_RAM_COST },
        ]);
    });
    ...
    test('includes zero-cost identifiers when they are detected (still triggers base cost)', () => {
        ...
        assert.equal(result.total, BASE_RAM_COST);
        assert.deepEqual(result.entries, [
            { name: 'write', cost: 0 },
            { name: BASE_COST_LABEL, cost: BASE_RAM_COST },
        ]);
    ...
    test('does not append base cost when no ns methods were detected', () => {
        ...
        const result = computeScriptRamCost('const x = 42; console.log(x);', costs);
        assert.deepEqual(result, { total: 0, entries: [] });
    });
    ...
    test('counts identifiers reached via bracket-string accessor', () => {
        ...
        const result = computeScriptRamCost(src, costs);
        assert.deepEqual(result.entries.map(e => e.name), ['grow', 'hack', BASE_COST_LABEL]);
    });
    ...
    test('counts identifiers reached via optional chaining (?.name)', () => {
        ...
        const result = computeScriptRamCost(src, costs);
        assert.deepEqual(result.entries.map(e => e.name), ['hack', BASE_COST_LABEL]);
    });
    ...
    test('matches whole identifiers only — a longer name does not trigger a shorter cost key', () => {
        ...
        const substringCosts = new Map<string, number>([
            ['grow', 0.15],
            ['growthAnalyze', 1],
            ['hack', 0.1],
            ['hackAnalyze', 1],
        ]);
        const result = computeScriptRamCost(
            'ns.growthAnalyze(server, 2); ns.hackAnalyze(server);',
            substringCosts,
        );
        const names = result.entries.map(e => e.name);
        assert.deepEqual(names, ['growthAnalyze', 'hackAnalyze', BASE_COST_LABEL]);
        // grow (0.15) and hack (0.1) must be absent from the total.
        assert.equal(result.total, 1 + 1 + BASE_RAM_COST);
    });
    ...
    test('does not match identifiers embedded inside a longer identifier', () => {
        ...
        const substringCosts = new Map<string, number>([['grow', 0.15]]);
        const result = computeScriptRamCost(src, substringCosts);
        assert.deepEqual(result, { total: 0, entries: [] });
    });
});
...
suite('formatRam', () => {
    test('always pads to two decimal places (matches Bitburner\'s in-game display)', () => {
        assert.equal(formatRam(0), '0.00 GB');
        assert.equal(formatRam(2), '2.00 GB');
        assert.equal(formatRam(2.5), '2.50 GB');
        assert.equal(formatRam(1.85), '1.85 GB');
        assert.equal(formatRam(0.05), '0.05 GB');
    });
